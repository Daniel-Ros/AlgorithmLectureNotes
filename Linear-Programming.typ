...
#v(-10pt)
#definition[
  A Linear program is a one where $bx in RR^n$ and one of the following 6-forms:
  #table(
    columns: (1fr, 1fr, 1fr),
    stroke: none,
    [$max{ bc^T bx : A bx <= bb }$], [$max{ bc^T bx : A bx <= bb, bx >= b0 }$], [$max{ bc^T bx : A bx = bb, bx >= b0 }$],
    [$min{ bc^T bx : A bx >= bb }$], [$min{ bc^T bx : A bx >= bb, bx >= b0 }$], [$min{ bc^T bx : A bx = bb, bx >= b0 }$],
  )
]

#definition[
  An Integer program is a one where $bx in ZZ^n$ and one of the following 6-forms:
  #table(
    columns: (1fr, 1fr, 1fr),
    stroke: none,
    [$max{ bc^T bx : A bx <= bb }$], [$max{ bc^T bx : A bx <= bb, bx >= b0 }$], [$max{ bc^T bx : A bx = bb, bx >= b0 }$],
...
- minimize $sum_(v in V) y_v = max b1^t by$. #h(1fr) #tr[(We want minimum vertex-cover)]

*Constaints:* $forall e in E: sum_(v in e) y_v >= 1$ #h(1fr) #tr[(Every edge is covered at least once)]


This vector $bx$ is the characteristic vector of a vertex cover in $G$.
...
#table(
  ...
  [ 
    ...
    [ 
      ...
      *LP* for minimum-vc
      $
        max_(by in RR^n){ b1^t by : M by >= 1, by>=0}
      $ 
    ]
  ), #[This called the: \ *Corresponding LP Relaxation*])$
  ]
...
#pause
      $
        min_( by in ZZ^n){ underbrace(bw^t by, 
        sum_(v in V) w_v dot y_v)  : M^t by >= b1, by>=b0}
      $ 
  ...
  [
    ...
    [ 
      ...
      *IP* for maximum matching
      #v(-10pt)
      $
        min_(bx in ZZ^n){ b1^t bx : M bx <= b1, bx>=b0}
      $ 
    ]
  )
  ],
  ...
  [
    ...
    [ 
      ...
      *LP* for maximum matching
      #v(-10pt)
      $
        min_(bx in RR^n){ b1^t bx : M bx <= b1, bx>=b0}
      $ 
    ]
  )
  ],
