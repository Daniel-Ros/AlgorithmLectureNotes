...
#block(fill: blue.lighten(80%), inset: 20pt, radius: 1em)[
  ...
  *For bipartite graphs $G:=(A cup.dot B, E)$*\
  #table(
    ...
    [$<==>$],
    [$|A| = |B|$\ and $A$ satisfoes the \ Hall-Condition]
  )
]
#question[
...
#place(
  ...
  #place(
    ...
    cetz-canvas({
      ...
      for i in range(2, 6){
        rect-around("o" + str(i), "o" + str(i), stroke: none, padding: (0.2, 0.2, 0.2, 0.2), radius: 0.1, fill: red.lighten(50%))
      }
 ...
 - if #text(fill: purple)[$e$ is an edge] that resides inside any component. inside of $S$, or between $S$ and any component.
  ...
  #place(
    ...
    cetz-canvas({
      ...
      for i in range(0,3){
        circle((calc.cos(i + 0.44) * 1.5 + 6, calc.sin(i  + 0.44) * 1.5 - 4),fill: red, radius: 3pt, name: "r" + str(i))
      }
      circle((4.5, 0.3), fill: black, radius: 3pt, name: "t0")
      circle((4, 1), fill: black, radius: 3pt, name: "t1")
...
#place(
    ...
    cetz-canvas({
      ...
      for i in range(0, k+1){
        for j in range(0,k){
          circle((calc.cos(j * 2 + i) + i * 5, calc.sin(j * 2 + i)), fill: black, radius: 3pt, name: str(j) + "l" + str(i))
        }
      }
